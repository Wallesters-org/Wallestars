...
# ⚠️ ПРЕДУПРЕЖДЕНИЕ ЗА СИГУРНОСТ: Променете 'changeme123' със силна парола!
# Генерирайте сигурна парола с: openssl rand -base64 32
```

**🔒 ВАЖНА БЕЛЕЖКА ЗА СИГУРНОСТ**: Примерът по-горе използва парола по подразбиране `changeme123` само за демонстрационни цели. Вие **ТРЯБВА** да я промените със силна, уникална парола преди deployment в production. Използвайте командата по-долу за генериране на сигурна парола:

```bash
# Генериране на сигурна случайна парола
